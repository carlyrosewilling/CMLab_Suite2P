...
[~, infonew] = read_patterned_tifdata(fs{1}(1).name); 
nFr = infonew.nframes;
Ly = infonew.tifinfo.ImageLength;
Lx = infonew.tifinfo.ImageWidth;
%nFr = length(Info0);
%Ly = Info0(1).Height;
%Lx = Info0(1).Width;
ops.Lx = Lx;
...
if ~isempty(targetImage)
    ...
    while indx < ops.NimgFirstRegistration
        ...
        if abs(nbytes - fs{k}(j).bytes)>1e3
            ...
            [~, tempinfo] = read_patterned_tifdata(fs{k}(j).name);
            nFr = tempinfo.nframes;
